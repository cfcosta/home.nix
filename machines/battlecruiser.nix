...
}:
{
  ...
  config = {
    ...
    home-manager.users.${config.dusk.username} = {
      ...
      # Mirror Eden (Switch) and RPCS3 (PS3) saves into RomM on the homelab VM.
      #
      ...
      # one level of regular files). Both emulators keep a DIRECTORY per save,
      # so this ships each one as a zip, which is the shape RomM hashes
      # member-wise and can therefore compare without a download.
      #
      ...
      systemd.user.services.romm-save-sync = {
        Unit.Description = "Mirror Eden and RPCS3 saves to RomM";

        Service = {
          Type = "oneshot";

          # ROMM_URL and the rmm_ client token live in a mode-600 file outside
          # the store and the repo, same as jobcrawler's daemon.env above.
